...
<p>不同于基于模拟环境的 CTF 比赛，本次大赛基于中国科学技术大学校内的真实网络服务，参赛者需要通过渗透靶标列表中的设备、应用或网站，发现并利用可对数据保密性、完整性或服务可用性造成破坏的安全漏洞。</p>
...
<li>5月30日排行榜封榜，取前5名参加决赛答辩。参加决赛答辩者需提前准备答辩材料。</li>
<li>6月6日举行决赛答辩，答辩现场根据比赛排名、渗透手法、漏洞价值，角逐出冠亚季军，颁发奖品及证书。</li>
...
<p>冠亚季军以决赛答辩的最终名次决定。</p>
<p><b>最佳创意奖：</b>组委会评出最具创意的渗透手法，参赛者可获得奖品及证书；</p>
<p><b>最佳女生奖：</b>排行榜排名最高的女生，可获得奖品及证书；</p>
<p><b>白帽子奖：</b>所有提交漏洞并审核通过的参赛者均可获得 LUG 专属白帽子一顶。</p>
...
<p>排行榜按照有效漏洞的得分之和排序。得分相同的，按照罚时从少到多排序。罚时是从比赛开始到每个漏洞提交的时间之和。</p>

<p>一个有效漏洞的得分等于靶标权重与漏洞 rank 的乘积。如果在同一个靶标发现了多个漏洞，只计 rank 最高的那个漏洞。如果一个漏洞影响了多个靶标，参赛者须对每个靶标分别提交报告，视为不同的漏洞。</p>
...
<li>靶标服务的开发者、管理员、维护者不准攻击自己参与开发或维护的服务，违者视为无效。</li>
</ol>
...
<table class="table table-striped table-bordered table-hover">
  ...
  <tbody>
    ...
    <tr><td>ustcnet 无线热点</td><td>对其他无线用户实施 ARP 欺骗</td><td>200</td></tr>
  </tbody>
</table>


      <h3>2）应用类</h3>
<p>靶标范围为掌上科大 http://i.ustc.edu.cn/m/ 上的应用。</p>
...
<p>漏洞 rank 由组委会评定。用户在提交漏洞时应当自评 rank 供参考。以下是部分漏洞类型的参考 rank：</p>
<ul>
<li>沙盒逃逸（20）</li>
<li>远程任意代码执行（20）</li>
<li>SQL 注入（15）</li>
<li>拒绝服务（要求攻击流量极小）（15）</li>
<li>用户权限提升（例如普通用户能执行管理员操作）（10）</li>
<li>用户信息泄露（例如查看其他用户的隐私数据）（10）</li>
<li>绕过登录认证（10）</li>
<li>管理员弱口令（10）</li>
<li>网站存在病毒、木马、垃圾信息（5）</li>
<li>浏览器 XSS、客户端代码注入（5）</li>
<li>浏览器 CSRF、点击劫持（3）</li>
<li>服务器敏感配置泄露（2）</li>
</ul>
    </div>
